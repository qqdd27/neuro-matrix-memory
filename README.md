...
| Session project-status + resolved Q&A + `invent` | — | session end → durable `kind='status'` rollup surfaced on the next session's first turn; the 2nd distinct ask of a question freezes the best answer (`kind='resolved'`, instant replay); `invent(goal)` proposes novel component pairs by graph-edge novelty, dead-end pairs excluded |
